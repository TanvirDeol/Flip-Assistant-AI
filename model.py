...
class KMeans:
  ...
  def get_sim_links(self,point,index,k=3):
    sim = []
    for i in self.classes[index]:
  ...
  def get_deal_links(self,point,index,k=3):
      sim = []
      for i in self.classes[index]:
        sim.append([i[0],-i[1],i[2]])
      sim.sort()
      links = []
      for i in range(k):
        links.append(sim[i][2])
...
def optimize_model(k,dlist,point):
...
def prepare_data():
  ...
  data = data.drop(columns=['color','shipping','import charges'])
  dlist = data.values.tolist()
  return [dlist,data]

def execute(point,type):
  dlist,data = prepare_data()
  k_means_clf = KMeans(k=len(dlist)//5)
  k_means_clf.train(dlist)
  index = k_means_clf.predict(point)
  links = []
  if type == 'Buy':
    links = k_means_clf.get_deal_links(point,index,k=min(3,len(k_means_clf.classes[index])))
  elif type =='Sell':
    links = k_means_clf.get_sim_links(point,index,k=min(3,len(k_means_clf.classes[index])))
  return [data,round(k_means_clf.get_price(index),2),links]

#point = [0,0,0,1,1,6,32]
#execute(point,'Buy')
